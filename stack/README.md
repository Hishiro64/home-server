# 📚 Stacks

* ✅ Current Services
  * [Homer](current/homer.yml)
  * [File Browser](current/filebrowser.yml) <!-- Could be better -->
  * [JellyFin](current/jellyfin.yml)
  * [Aria2 Pro](current/aria2-pro.yml)
  * [Scrutiny](current/scrutiny.yml)
  * [Netdata](current/netdata.yml) <!-- Will force a cloud requirement! -->
  * [Dozzle](current/dozzle.yml)
  * [WireGuard-Easy](current/wg-easy.yml)
  * [qBittorrent](current/qbittorrent.yml)
  * [Gitea](current/gitea.yml)
  * [Stash](current/stash.yml)
  * [LanguageTool](current/languagetool.yml) <!-- Version 8.13.2 of the Firefox add-on -->
  * [Syncthing](current/syncthing.yml)
  * [QDirStat](current/qdirstat.yml)
  * [Doku](current/doku.yml)
  * [Jellyseerr](current/jellyseerr.yml) <!-- Would perfer an alternative -->
  * [Samba](current/samba.yml)
  * [LANraragi](current/lanraragi.yml)
  * [Calibre Web Automated](current/calibre-web-automated.yml)
  * [Memos](current/memos.yml)
  * [Immich](current/immich.yml)
  * [Kiwix-Serve](current/kiwix-serve.yml)
  * [Snippet Box](current/snippet-box.yml) <!-- Need alternative, unmaintained, CVE-2023-23277 -->
* ❌ Retired Services 
  * [Duplicacy](retired/duplicacy.yml)
  * [Kavita](retired/kavita.yml)
  * [Pi-hole](retired/pi-hole-vanilla.yml) <!-- Dont really use it, ublock on firefox, no smart devices, etc-->
  * [ByteStash](retired/bytestash.yml) <!-- Kinda worse than snippet-box at the moment-->
  * [IT-Tools](retired/it-tools.yml) <!-- Don't find myself using it -->
  * [Tube Archivist](retired/tube-archivist.yml) <!-- RAM intensive, would like an alternative, otherwise use Stash -->
  * [Watchtower](retired/watchtower.yml) <!-- Update manually now for reliability now -->
  * [Pi.Alert](retired/pi.alert.yml) <!-- Alternative fork available -->
  * [Dashdot](retired/dashdot.yml) <!-- Quite CPU intensive -->
  * [Komga](retired/komga.yml) <!-- Not much better than Kavita, everything other than *.cbz files is slow -->